...
{
	unsigned char Digit;
	static unsigned char Xposition;
	Xposition = Xpos;
	OLED_SetCursor(Ypos,Xpos);
	
	if (Number > 9) {
		OLED_WriteNumber(font,Xpos,Ypos,Number/10);
	}
	
	OLED_WriteChar(font,Xposition,Ypos,(Number%10)+0x30,false,false,false);
	Xposition+=font.fontWidth;
